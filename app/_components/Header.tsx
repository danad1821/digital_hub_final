...
export default function Header({ scrollToSection }: HeaderProps) {
  ...
  const dynamicClasses =
    scrolled || isOpen ? solidStateClasses : transparentStateClasses;
          ...
          <div className="hidden lg:flex items-center justify-center gap-6">
            {NAV_LINKS.map((link) => {
              ...
              const regularLinkClasses = `transition duration-150 cursor-pointer ${
                scrolled || isOpen
                  ? "text-gray-800 hover:text-[#00D9FF]"
          ...
          <div className="lg:hidden">
            <button onClick={toggleSidebar} className="py-2 text-white">
              {isOpen ? (
                ...
                <RxHamburgerMenu
                  className={`text-3xl ${scrolled || isOpen ? "text-black" : "text-white"}`}
                />
              )}
            </button>
          </div>
                ...
                <li key={link.name}>
                  ...
                  <button
                    ...
                    className={`block py-2 transition duration-200 text-base w-full text-left
                      ${
                        isCTA
                          ? "bg-[#00D9FF] flex items-center justify-center text-gray-800 font-medium w-full text-center rounded-sm mt-2 hover:bg-[#00D9FF]/80"
